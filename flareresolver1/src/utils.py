...
def _build_proxy_address(proxy: dict) -> str | None:
    ...
    if proxy_url:
        parsed_proxy_url = urllib.parse.urlsplit(proxy_url)
        if not parsed_proxy_url.scheme or not parsed_proxy_url.hostname:
            logging.warning("Ignoring invalid proxy url: %s", proxy_url)
            return None
        return _apply_proxy_credentials(parsed_proxy_url, proxy)

    if all(key in proxy for key in ['host', 'port']):
...
def _apply_proxy_credentials(parsed_proxy_url, proxy: dict) -> str:
    username = proxy.get('username', parsed_proxy_url.username)
    password = proxy.get('password', parsed_proxy_url.password)
    hostname = parsed_proxy_url.hostname or ''
    port = proxy.get('port', parsed_proxy_url.port)

    if hostname and "pyproxy" in hostname and username:
        proxy_session = random.randint(0, 99999999)
        username = username + f"-session-{proxy_session}-sessTime-10"

    netloc = hostname
    if username is not None:
        quoted_username = urllib.parse.quote(str(username), safe='')
        if password is not None:
            quoted_password = urllib.parse.quote(str(password), safe='')
            netloc = f"{quoted_username}:{quoted_password}@{netloc}"
        else:
            netloc = f"{quoted_username}@{netloc}"
    if port:
        netloc = f"{netloc}:{port}"

    return urllib.parse.urlunsplit((
        parsed_proxy_url.scheme or 'http',
        netloc,
        parsed_proxy_url.path,
        parsed_proxy_url.query,
...
def get_webdriver(proxy: dict = None) -> WebDriver:
    ...
    proxy_address = _build_proxy_address(proxy)
