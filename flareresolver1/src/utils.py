...
def get_webdriver(proxy: dict = None) -> WebDriver:
    ...
    if proxy and all(key in proxy for key in ['host', 'port', 'username', 'password']):
        if "pyproxy" in proxy['host']:
            proxy_session = random.randint(0, 99999999)
            proxy['username'] = proxy['username'] + f"-session-{proxy_session}-sessTime-10"
        proxy_address = f"http://{proxy['username']}:{proxy['password']}@{proxy['host']}:{proxy['port']}"
    elif proxy and all(key in proxy for key in ['host', 'port']):
