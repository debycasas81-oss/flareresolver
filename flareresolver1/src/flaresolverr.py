...
env_proxy_host = os.environ.get('PROXY_HOST', None)
env_proxy_port = os.environ.get('PROXY_PORT', None)
...
def controller_v1():
    ...
    if (('proxy' not in data or not data.get('proxy')) and env_proxy_host is not None and env_proxy_port is not None and (env_proxy_username is None and env_proxy_password is None)):
        logging.info('Using proxy host & port ENV')
        data['proxy'] = {"host": env_proxy_host, "port": env_proxy_port}
    if (('proxy' not in data or not data.get('proxy')) and env_proxy_host is not None and env_proxy_port is not None and (env_proxy_username is not None or env_proxy_password is not None)):
        logging.info('Using proxy host & port, username & password ENVs')
        data['proxy'] = {"host": env_proxy_host, "port": env_proxy_port, "username": env_proxy_username, "password": env_proxy_password}
...
@app.post('/postman')
def controller_postman():
    """
    Controller Postman
    """
    data = request.json or {}
    data['cmd'] = 'request.get'
    data['url'] = 'https://identity.getpostman.com/login'
    data['maxTimeout'] = 120_000
    # block images/css/fonts to speed up page load (caller may override)
    if 'disableMedia' not in data:
        data['disableMedia'] = True
    # pass credentials via dedicated field (postData conflicts with GET validation)
    if 'line' in data:
        data['credentials'] = data.pop('line')
    if (('proxy' not in data or not data.get('proxy')) and env_proxy_host is not None and env_proxy_port is not None and (env_proxy_username is None and env_proxy_password is None)):
        logging.info('Using proxy host & port ENV')
        data['proxy'] = {"host": env_proxy_host, "port": env_proxy_port}
    if (('proxy' not in data or not data.get('proxy')) and env_proxy_host is not None and env_proxy_port is not None and (env_proxy_username is not None or env_proxy_password is not None)):
